...
func TestFederationRestrictedMediaUnstable(t *testing.T) {
	...
	t.Run("TestInvitedVisibilityMediaMessage", func(t *testing.T) {
		...
		// bob should have the right to see that media. This only seems to work in this test because of our sentinel user
		// above, "tank". Otherwise the event message would not make it across the federation and bob would not be able to
		// see it. The argument that this is perfectly fine, as otherwise how would bob have known the mxcUri anyway
		bob.DownloadContentAuthenticated(t, mxcUri)
